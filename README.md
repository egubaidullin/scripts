...
| **[ufw-sync.sh](ufw-sync/ufw-sync.sh)** | This script configures UFW (Uncomplicated Firewall) by detecting the current IP, resetting UFW settings, and applying rules for SSH, Docker, and Cloudflare IPs. It also integrates with Fail2Ban for additional security. [Learn more](ufw-sync.md). |
